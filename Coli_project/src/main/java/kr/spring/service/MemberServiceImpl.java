...
public class MemberServiceImpl implements MemberService {
    ...
    public void modify(Member vo, HttpSession session) {
        ...
        Member existingMember = member.orElse(null);
        if (existingMember != null) {
            ...
            // 비밀번호가 입력되었다면 확인 후 인코딩하여 저장
            if (vo.getPassword() != null && !vo.getPassword().isEmpty()) {
                if (isPasswordMatch(vo.getPassword(), vo.getPasswordConfirmation())) {
                    String encPw = vo.getPassword();
                    existingMember.setPassword(passwordEncoder.encode(encPw));
                } else {
                    throw new PasswordNotMatchException("비밀번호와 비밀번호 확인이 일치하지 않습니다.");
                }
            }
            
            // 비밀번호 확인이 입력되었다면 확인 후 인코딩하여 저장
            if (vo.getPasswordConfirmation() != null && !vo.getPasswordConfirmation().isEmpty()) {
                if (isPasswordMatch(vo.getPasswordConfirmation(), vo.getPassword())) {
                    String encPwConfirmation = vo.getPasswordConfirmation();
                    existingMember.setPasswordConfirmation(passwordEncoder.encode(encPwConfirmation));
                } else {
