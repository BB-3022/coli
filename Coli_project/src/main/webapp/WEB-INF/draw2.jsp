...
<link rel="stylesheet" type="text/css" href="${cpath}/css/style2.css">
   ...
   <section id="login-intro" class="login-section"">
      <div class="log-in"">
         ...
         <div class="grid-container">
          ...
          <div class="sidebar">
    <button id="click-button" title="클릭형 자세 만들기" onclick="handleClickButton()">
        <i class="fa-solid fa-arrow-pointer"></i>
    </button>
    <button id="upload-button" title="이미지 업로드 자세 만들기" onclick="handleUploadButton()">
        <i class="fa-solid fa-images"></i>
    </button>
    <button id="capture-button" title="실시간 자세 만들기" onclick="handleCaptureButton()">
            ...
            <div id="click-content" class="content-section ">
               <div class="grid-item-group click-layout">
                  ...
                  <div class="grid-item">
                     ...
                     <button id="captureButton" class="btn_download" onclick="saveImage()">Save Image</button>
    ...
    <script type="text/javascript">
	    var userRole = '${user.member.role}'; // 사용자 role 설정
	    var isLoggedIn = ${not empty user}; // 로그인 여부 확인
	
	    function handleClickButton() {
	        window.location.href = '${cpath}/draw2';
	    }
	
	    function handleUploadButton() {
	        if (isLoggedIn && userRole !== 'free') {
	            window.location.href = '${cpath}/draw3';
	        } else {
	            alert('이 기능을 사용하기 위해서는 상위 플랜이 필요합니다.');
	            window.location.href = '${cpath}/plan'; // 권한이 없을 때 리디렉션할 페이지
	        }
	    }
	    ...
	    function handleCaptureButton() {
	        if (isLoggedIn && userRole === 'premium') {
	            window.location.href = '${cpath}/draw4';
	        } else {
	            alert('이 기능은 프리미엄 플랜에서만 사용 가능합니다.');
	            window.location.href = '${cpath}/plan'; // 권한이 없을 때 리디렉션할 페이지
	        }
	    }
