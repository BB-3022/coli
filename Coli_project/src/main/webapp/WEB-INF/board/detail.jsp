...
<body>
		...
		<div class="detail-form xans-element- xans-board xans-board-read-4 xans-board-read xans-board-4" style="overflow-y: hidden; margin: 0 390px 0 390px;">
			<div class="ec-base-table typeWrite ">
				<table border="1">
					...
					<tbody>
						...
						<tr>
							<th scope="row">작성자</th>
                    		<td>${article.writer_id.username}</td>
                		</tr>
                		<tr>
							<td colspan="2">
                        		<ul class="etcArea">
            ...
            <!-- 로그인한 사람과 작성자 아이디가 같아야만 보이도록 -->
			<c:choose>
			    <c:when test="${user.member.username eq article.writer_id.username}">
			        <span class="gRight">
			            <a href="${cpath}/board/modify?atc_id=${article.atc_id}&user_id=${article.writer_id.username}" class="Button button-rounded button-small">수정</a>
			            <a href="${cpath}/board/remove?atc_id=${article.atc_id}&user_id=${article.writer_id.username}" class="Button button-rounded button-small">삭제</a>
			        </span>
			    </c:when>
			</c:choose> 
            <!-- 로그인한 사람과 작성자 아이디가 같아야만 보이도록 -->
					    ...
					    <c:when test="${not empty comments}">
					        <c:forEach var="comment" items="${comments}">
					            <li class="first xans-record-">
					                ...
					                <span class="button">
					                    <c:if test="${user.member.username eq comment.cmt_writer_id.username}">
